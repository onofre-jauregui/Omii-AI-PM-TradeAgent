...
**Health snapshot (live `compliance_log`, trailing 72h):** 0 `critical`, 1 `error` — a single self-healing `kalshi-proxy exception: error reading a body from connection` at 07-22 07:35 UTC. `health_check_alert` pages in the trailing 14 days: 8 total, 6 of them `api_error_kalshi` fired off a **single** transient event each (429s and one connection-read exception on 07-12, 07-18 ×2, 07-19, 07-20, 07-22 ×2), 2 `trading_silence` (expected paper-mode notices), 2 `system_errors`. The two `market_data_fetcher_aborted` **critical** pages (07-13, 07-16 — run-budget exceeded before a single series fetched, likely `getKalshiCredentials()` hanging ahead of the per-series budget check in `market-data-fetcher/index.ts`) have not recurred in 6 days and are a separate, lower-frequency issue — not this entry's focus.

**The finding (both previously-logged, still-open bugs reproduced live this run):**
1. **Double-page confirmed again, fresh instance:** the 07:35:06 UTC `api_error` row logged at `severity: "error"` matches *both* sweeps in `health-check/index.ts` — block 8 (`severity IN ('error','critical')`, ~line 252) and block 9 (structured `API_ERROR_TYPES` sweep, ~line 307-353) — and both fired at the next health-check run: `health_check_alert` rows for `api_error_kalshi` and `system_errors` landed within 0.36s of each other, 08:10:08.935 and 08:10:09.293 UTC. This is the exact mechanism `docs/improvement-log.md`'s own top entry (07-10 2nd) diagnosed 12 days ago, with a fully-specified one-line fix (`.not("event_type","in","(api_error,llm_rate_limit,api_timeout,kalshi_circuit_open)")` on block 8's query) that has sat unshipped since.
2. **No sustained-condition gate, reproduced 6x in 8 days:** every `api_error_kalshi` page this window was triggered by a single occurrence (count=1) of a transient, already-retried Kalshi hiccup (429 or connection-read exception) — `market-data-fetcher`'s own `fetchWithRetry` (3 retries, exponential backoff) already exhausted before logging, so by definition these are self-healing-or-already-failed-over blips, not sustained outages. `pendingAlerts.push` at `health-check/index.ts:345-353` pages on any `count ≥ 1` in the trailing 2h window — there is no `count >= 2` (or cross-cycle persistence) gate. This is the same root cause `docs/improvement-log.md`'s 07-09 (2nd) entry logged and proposed fixing.

**Root cause:** two fully-diagnosed, fully-specified fixes from prior scheduled health checks (07-09 2nd, 07-10 2nd) remain uncommitted. Nothing new is broken; the existing findings keep re-manifesting because they're gated on a human "go" that hasn't been given, and each recurrence costs a real Telegram interrupt with zero new information.

**Fix (proposed, unchanged from prior entries — restating because it's still the highest-leverage single change):**
1. Ship the block-8 scope fix from 07-10 (2nd) — stops every real API error from double-paging.
2. Add a `count >= 2` (or equivalent 2-cycle persistence) gate to the block-9 `api_error_*` sweep before paging, per 07-09 (2nd) — stops isolated, already-retried blips from paging at all.
3. Both fixes are small, independently deployable, and already reviewed in this log — no new design work required, only the deploy.

**Why it matters ($ / revenue):** 6 individual-blip pages + 1 double-page in 8 days on a channel that must stay trustworthy for the one outage that actually threatens trades (no data → no trades → no track record, the artifact gating the uncle-capital unlock). Every no-op page trains the reader to skim past Telegram alerts, which is the exact failure mode that would let a real outage go unnoticed. The fix cost is near zero (both are already written, reviewed, and sitting in this log) — the only blocker is the deploy approval.

**Verification plan before shipping:** deploy both fixes together; confirm the next `severity=error` API-error row produces exactly one page; confirm a single isolated 429/timeout no longer pages at all, but 2+ occurrences of the same provider:status within the 2h window still does.
...
## 2026-07-23 — `market-data-fetcher`'s cache-eviction guard is blind to budget-skipped series → a second consecutive abort would delete live market data, not just serve stale data

**Status:** Logged — not yet fixed/deployed (production deploy is a Hard Stop; awaiting go). Found via scheduled health check.

**Health snapshot (live `compliance_log`):** no unresolved errors — a real Kalshi 503 degradation fired 46 `api_error` rows between 07:03–07:46 UTC (self-healing, same class as the already-logged "07-09 (2nd)" transient-blip finding) and one genuine `critical` page fired clean at 15:11:53 UTC: `market_data_fetcher_aborted` — "Reason: run budget exceeded (53.1s), 0 series failed, 10 skipped: KXNBA, KXMLB, KXCHCUTS, KXBTCD, KXETHD, KXHIGHNY, KXHIGHCHI, KXHIGHMIA, KXHIGHLAX, KXHIGHAUS." The alert worked exactly as designed (fired immediately via Telegram, not queued for the hourly sweep) and the run self-healed one cycle later (15:16 UTC: 18/18 series OK). No action needed on the alerting path itself — the finding is in what runs *after* an abort.

**The finding:** `market-data-fetcher/index.ts:209` guards cache eviction with `if (failedSeries.length === 0)` — the comment above it (`:206-208`) says the intent is "rows not refreshed this cycle are closed/settled markets, safe to delete." That's true when every series was *attempted* and none failed. It is false on a budget-abort cycle: `failedSeries` stays empty (nothing errored — the run just ran out of time), but `skippedSeries` holds up to all 18 series that were **never attempted**. The guard doesn't check `skippedSeries.length`, so a budget-abort cycle still passes `failedSeries.length === 0` and proceeds to delete every `kalshi_markets_cache` row older than 15 minutes — including rows for the series that were skipped, not closed.

This has not yet caused data loss because all 3 known aborts (07-13, 07-16, today) self-healed on the very next 5-min cycle, so the skipped series' cache rows were still <15 min old when eviction ran. But this is a single self-healing cycle away from becoming a real one: if a budget-abort recurs on 2+ **consecutive** cycles (10+ min with zero refresh for the same tail series — plausible under sustained Kalshi latency, exactly the kind of condition that just occurred this morning at 07:03–07:46 UTC), eviction would delete live, open markets' cache rows for every series past the point the run choked, not because they closed but because nobody re-fetched them in time. Downstream, `surface-scanner` and `weather-signal` read `kalshi_markets_cache` — for those series they'd see **zero markets** (looks like "series doesn't exist") rather than "stale by one cycle," which is a worse failure mode than the "stale data" the abort alert already warns about.

**Root cause:** the eviction guard's correctness assumption — "no failures ⇒ any unrefreshed row must be a closed market" — was written before the run-budget abort path existed, and was never updated to account for series that are skipped outright rather than attempted-and-failed. `failedSeries` and `skippedSeries` are tracked separately everywhere else in the file (both are reported in the abort alert and the terminal `market_data_fetch` log line) but only one of the two gates eviction.

**Fix (proposed, one line):** at `market-data-fetcher/index.ts:209`, change the guard to `if (failedSeries.length === 0 && skippedSeries.length === 0)`. On any aborted run, skip eviction entirely for that cycle — the next successful full run will naturally re-evict anything genuinely closed.

**Why it matters ($ / revenue):** this is the sole Kalshi market-data poller — every downstream consumer (surface-scanner, signal-generator, weather-signal) reads only from this cache and never calls Kalshi directly. A silent gap that empties cache rows for live, tradeable series looks identical to "market closed" to every consumer, with no distinguishing signal — worse than the stale-data risk the abort alert already flags, and currently undetectable except by noticing a series' alert volume drop to zero. One-line fix, zero trading-behavior change on the non-abort path.

**Verification plan before shipping:** after the change, simulate two consecutive budget-abort runs (or replay recorded timing) and confirm `kalshi_markets_cache` rows for the skipped series are *not* deleted between them; confirm eviction still fires normally and prunes truly-closed markets on a clean (`failedSeries.length === 0 && skippedSeries.length === 0`) run; confirm the abort alert and terminal log line are unaffected.

## 2026-07-23 (2nd) — Two open config questions, requested by Onofre: is `auto-trade-cron`'s hourly cadence the right value, and is `max_open_positions=4` the right value (root cause of the 3-day trading freeze in the 07-23 (1st) entry above)

**Status:** Investigation/documentation entry — no config or code changed. Both items below are decisions for Onofre to confirm; changing a live pg_cron schedule or a production `risk_settings` value is treated as a production change (Hard Stop), not auto-applied. `docs/system-report.md`'s Scheduled Jobs table was corrected in this same pass (see below) — that part is a pure doc fix, safe to ship without a decision.

**1. `auto-trade-cron` cadence — no documented rationale exists for "hourly"; the project's own docs and code comments both disagree with production reality.**

Verified live (`cron.job`, 2026-07-23): `auto-trade-cron` runs `5 * * * *` — once per hour. Three independent sources describe a *different* cadence:
- `docs/system-report.md:252` (pre-fix) claimed "Every 2 minutes" — wrong by 30x, and stale since at least whenever that doc was last touched.
- `auto-trade/index.ts:1622` and `:2252` both say a signal "is re-qualified on every 30s run until the freshness window expires" — also wrong by 120x against the real hourly cadence.
- Git history shows real drift, not a single deliberate choice: `20260406_signal_surface_tables.sql` (Apr 7) set it to every 5 min with the comment "the agent loop itself takes 30-90 seconds to run per strategy"; `b70b829` (May 12) tightened it to `'30 seconds'` via `cron.schedule`, adding an `auto_trade_locks` table specifically to guard against overlapping runs at that cadence; by `20260523_signals_acted_on_at.sql`'s companion commit (May 25, `9d7021c`) it was already hourly, staggered only for minute-offset. **No migration file, commit message, or `DECISIONS.md`/`improvement-log.md` entry between May 12 and May 23 records the change from 30-second to hourly or why.** The two most plausible causes in that window — `d980458` (May 21, Kalshi rate-limit cache layer) and `8e8a36c` (per-user rate limiting, 3/min live · 15/min paper) — are real candidates by proximity, but neither commit's diff actually touches `auto-trade-cron`'s schedule, so this is a plausible inference, not a confirmed cause.

What this means practically: `market-data-fetcher` (every 5 min) already decouples Kalshi rate limits from execution frequency — `surface-scanner`, `signal-generator`, and `futures-signal` all read the shared cache, not Kalshi directly, so they don't need auto-trade's cadence to protect the rate limit. `execute-trade` *does* call Kalshi directly (`POST /portfolio/orders`, `execute-trade/index.ts:565`), but only when a strategy actually decides to fire — a `no_setup` tick costs zero Kalshi calls. Cost-wise, each hourly run resolves one AI config and each of the 3 strategies (S-001/S-002/S-005) makes its own qualify call — roughly 3 LLM calls/hour (~72/day) today; matching the scanner's 5-min cadence would be ~12x that (~864/day). Given the signals underneath already refresh every 5-15 min, an hourly execution check means a fast-moving arb edge (surface-scanner finds new alerts every 5 min) can open and fully close within the hour and never be seen by auto-trade at all — the freshness-window logic in the code was written assuming it would be re-evaluated far more often than it actually is.

**Recommendation (needs Onofre's go, not applied):** this isn't settled by evidence alone — it's a real tradeoff between missed-edge cost (hourly) and LLM-cost + Kalshi-load (faster). Given the rate-limit-sensitive work is already isolated to the cache layer, a 5-10 min cadence for `auto-trade` (matching `surface-scanner`/`futures-signal`) is the more defensible default unless there's a cost or stability reason to stay hourly that hasn't surfaced in any doc, commit, or log — worth deciding once, in writing, either way.

**2. `max_open_positions=4` (paper, Onofre's account) — stale value from onboarding, counts legs not baskets, scoped globally instead of per-strategy. This is the exact mechanism behind the 3-day trading freeze in the entry directly above.**

Verified live: `risk_settings` for `user_id=ea207ba1...`, `mode=paper`: `max_open_positions=4`, `updated_at=2026-05-26 09:57:13` — set once at onboarding, untouched since, while the system-wide default (`20260405165717_bf64e3a2...sql`, `20260402000000_kalshi_upgrade.sql`) is `10`. Three compounding issues, all confirmed in `auto-trade/index.ts:212-260,675`:
- `countOpenPositions()` counts individual `trades` rows with `status='filled' AND exit_reason IS NULL AND settled_at IS NULL` — i.e. **legs**, not baskets. S-001 Surface Arb places 3-6 legs atomically per opportunity (confirmed in `compliance_log`, 07-17 20:05: one run filled 6 legs across two brackets). A single basket can exceed a cap of 4 on its own.
- The call site (`:675`, `countOpenPositions(supabase, undefined, 7, strategy.user_id)`) passes `strategyId=undefined` — the cap is evaluated **per user, across all strategies**, not per strategy. So S-001 filling a basket blocks S-002 and S-005 too, even though they share nothing but the same risk-settings row.
- The cap was set to 4 on 2026-05-26, before S-001's atomic multi-leg basket execution was the routine behavior it is today — it was never revisited against that behavior.

**Recommendation (needs Onofre's go, not applied):** two independent fixes, either alone would have prevented the 3-day freeze: (a) raise `max_open_positions` to a value that comfortably fits at least one full basket per strategy with headroom (e.g. 15-20, given a basket can be 6 legs and 3 strategies share the cap), and/or (b) change `countOpenPositions` to scope per-strategy (pass `strategy.id` instead of `undefined`) so one strategy's basket can't freeze the other two. (b) is the more durable fix — it removes the cross-strategy coupling that caused this incident regardless of what the numeric cap is set to; (a) alone just moves the same failure mode to a higher position count.

**Why it matters ($ / revenue):** both values gate whether the agent trades at all. The cadence question determines how much real edge is missed between checks (direct P&L impact, unmeasured until decided and tested). The position-cap bug already cost 3 full days of zero trades across all three strategies — with no error, no alert, and no visible symptom short of noticing the trade table went quiet, which is exactly the failure mode the product's track-record thesis depends on not having.

**Verification plan before shipping either fix:** cadence — after changing the schedule, confirm `auto_trade_run` compliance_log rows land at the new interval with no overlap (the existing `auto_trade_locks` table already guards this) and that per-day AI-call volume moves as predicted; position cap — after raising the value and/or scoping per-strategy, confirm a single S-001 basket no longer blocks S-002/S-005's `risk_check_failed` checks, and confirm the cap still triggers correctly once genuinely exceeded (don't disable the guard, just fix its scope/value).

## 2026-07-23 (3rd) — Real cost-difference number for hourly vs 30s (negligible), and the actual answer to "is our observability system tracking cost": yes for auto-trade, but auto-reflect + compact-memory's LLM calls were invisible until this pass. Fix written, not deployed.

**Status:** Investigation complete, fix written on branch `fix/llm-cost-observability` (off `dev`) — not deployed. Onofre asked directly: (1) what's the real $ cost difference between hourly and 30-second `auto-trade` cadence, and (2) make sure the system's own cost observability is actually the source of truth, since "that is why we built it." Both answered with real data, not estimates.

**1. Real $ cost difference, hourly vs 30s — verified from live data, not modeled.** The system already logs real per-call token usage for `auto-trade`'s qualify LLM calls to `compliance_log` (`event_type='llm_usage'`), and the frontend `ObservabilityPage.tsx` already has a real per-model cost table (`MODEL_COSTS`, e.g. `openai/gpt-4o-mini`: $0.15/1M input, $0.60/1M output — OpenRouter's actual published rate, not a guess). Querying that table directly: **659 real qualify calls between 2026-06-01 and 2026-07-07**, averaging **725 input / 32 output tokens/call** → **$0.000128/call**. At today's hourly cadence that's ~18.3 calls/day ≈ **$0.0023/day (~$0.07/month)**. Scaling naively by the full 120x tick-count multiplier (worst case — assumes every extra tick reproduces a call at the same rate, which overstates it per the 07-23 (2nd) entry's freshness-window analysis) gives **~$0.28/day (~$8.40/month)** at 30-second cadence. **Conclusion: LLM cost is not a real constraint on cadence either way, at gpt-4o-mini volumes.** If hourly is being kept for a reason, that reason has to be Kalshi API load or execution stability — not $ cost — and neither of those has a documented rationale either (see 07-23 (2nd) entry).

**2. Why `llm_usage` has been silent since 07-07 — confirmed NOT a broken pipe.** Checked whether telemetry itself was broken (it would have to be, if trades were still executing "silently"): zero `auto_trade_strategy_error`/`api_timeout` rows since 07-07 (one self-healing timeout on 07-09), and every `trade_executed` row since 07-07 is **S-001 Surface Arbitrage only** — and S-001 is explicitly rule-based with **"no LLM gate needed"** (`auto-trade/index.ts:1257`, comment on the execution step). S-002/S-005 (the two strategies that *do* call the LLM) were rejected pre-LLM by their own rule-based filters through 07-20 (consistent with the already-logged finding that S-002 has gone 54 days without a trade before, `2026-07-08` entry above), then fully blocked by the `max_open_positions` risk-gate bug from 07-20 onward (07-23 1st/2nd entries) — which fires *before* the strategy handler is ever called, so it never reaches the LLM either. **The `llm_usage` pipe is verified correctly wired end-to-end** (real `data.usage.prompt_tokens`/`completion_tokens` extracted from the actual API response, not estimated) — it went quiet because real usage was genuinely zero, for reasons already on record elsewhere in this log.

**3. The actual gap — confirmed by sweeping every edge function for LLM call sites against logging coverage:**

| Function | LLM call sites | `llm_usage`-logged before this fix |
|---|---|---|
| `auto-trade` (qualify) | 1 | ✅ yes |
| `trading-agent` (chat panel) | 2 live + 1 dead (`streamAnthropicAsSSE`, defined, never called) | ✅ yes, both live paths (`chat_llm_usage`) |
| `auto-reflect` (lesson generation) | 1 | ❌ **no** |
| `compact-memory` (summarize + merge, called hourly from `auto-reflect`) | 2 | ❌ **no** |
| `list-ai-models` (provider-availability probe) | 1 | ❌ no — but `max_tokens:1`, functionally free, user-triggered only on key-add. Deliberately skipped, not fixed. |

`auto-reflect` runs hourly and calls an LLM (`gpt-4o-mini` via OpenRouter) to generate a causal lesson for every settled trade, then calls `compact-memory` (also hourly-gated, 30-min cooldown) which makes up to 2 more LLM calls (batch-summarize + cluster-merge) — all real, recurring spend that the cost dashboard has never seen, since these three call sites never wrote to `compliance_log`. This is the real answer to Onofre's question: **the observability system was not fully tracking cost** — not because of a bug in what it does track, but because three live LLM call sites were never wired into it.

**Fix (written, not deployed):** added the identical `llm_usage` `compliance_log` insert (same event shape as `auto-trade`'s: `model`, `provider`, `prompt_tokens`, `completion_tokens`, `total_tokens`, plus a `source` field to distinguish the emitter) to all three call sites — `auto-reflect/index.ts` (lesson generation, tagged `source: "auto-reflect-lesson"`) and `compact-memory/index.ts` (summarize + merge, tagged `source: "compact-memory-summarize"` / `"compact-memory-merge"`). Using the same `event_type` means `ObservabilityPage.tsx`'s existing `loadRealTokenStats()` (queries `compliance_log` where `event_type='llm_usage'`, no other filter) picks these up automatically — **zero frontend changes required**, the dashboard's `totalCost30d` figure was simply undercounting before this and will be complete once deployed. Fire-and-forget (`.then(undefined, () => {})`), matching the existing non-blocking observability pattern — cannot delay or fail a trading/reflection run.
