...
export function StrategiesProvider({ children }: { children: ReactNode }) {
  ...
  const loadStrategies = useCallback(async () => {
    ...
    if (!error && data) {
      setStrategies(data.map(s => ({
        ...
        run_interval_minutes: s.run_interval_minutes ?? null,
      })));
    } else {
      // Fallback defaults if DB not ready — display only, not used for routing
      setStrategies([
        {
          id: "S-001", template_id: "S-001", name: "Surface Arbitrage", mode: "paper", starting_balance: 500, active: true, run_interval_minutes: null,
        ...
        {
          id: "S-002", template_id: "S-002", name: "Resolution Fade", mode: "paper", starting_balance: 1000, active: true, run_interval_minutes: null,
        ...
        {
          id: "S-003", template_id: "S-003", name: "Economic Consensus", mode: "paper", starting_balance: 1000, active: true, run_interval_minutes: null,
        ...
        {
          id: "S-004", template_id: "S-004", name: "Liquidity Provision", mode: "paper", starting_balance: 1000, active: false, run_interval_minutes: null,
  ...
  const addStrategy = useCallback(async (strategy: Omit<Strategy, "id">) => {
    ...
    await supabase.from("strategies").insert({
      ...
      run_interval_minutes: strategy.run_interval_minutes ?? null,
