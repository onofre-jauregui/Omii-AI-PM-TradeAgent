...
const FAILURE_MODE_DETAILS: Record<string, { title: string; description: string; resolution: string }> = {
  ...
  execution_gap: {
    ...
    description: "The agent went completely silent — no run or skip events logged for more than 2 hours. This is a liveness signal, not a specific error type. Causes include: pg_cron stopped firing, the edge function crashed before it could write to the DB, or the DB itself was unreachable.",
...
export default function ObservabilityPage() {
  ...
  // Agent heartbeat — detects silence windows using a two-query approach.
  // Query 1: most recent event ever (unbounded) — catches ongoing silence since last run.
  // Query 2: last 24h events — computes median cadence and historical gaps within that window.
  // This fixes the blind spot where data.length < 2 in the 24h window returned [] even
  // when the agent had been completely silent for days.
  const loadExecutionGaps = useCallback(async () => {
    ...
    // Flag ongoing silence: last event more than 2 hours ago
    ...
    if (silenceMins > 120) {
      gaps.push({ from: latestRow?.created_at ?? since7d, to: nowIso, gapMins: silenceMins });
      setGapEvents(gaps);
      return;
    }

    // Enough recent events — compute median cadence and flag historical gaps
    ...
    const threshold = Math.max(15, median * 3);
    for (let i = 1; i < recent.length; i++) {
