...
| EF10 | `execute-basket` | HTTP, only live caller is `trading-agent`'s tool (auto-trade's S-001 routing comment is stale/false) | Given a leg failure after fills, `flattenFilledLegs()` runs and `baskets.status` reflects actual flatten success | none | **HIGH-STAKES + confirmed bug** — `finalStatus="flattened"` set from `flattenResults.length>0` without checking any flatten attempt succeeded; can mask real naked live exposure |
...
| EF10 `execute-basket` | `supabase/functions/_shared/basket-logic.test.ts` | `determineBasketStatus`'s 5 terminal-state branches, and `resolveFlattenOutcome`'s all-succeeded/partial-failure/total-failure/undercounted cases (finding #3 regression lock). 10 tests. |
