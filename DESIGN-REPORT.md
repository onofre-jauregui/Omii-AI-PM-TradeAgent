...
| EF10 | `execute-basket` | HTTP, only live caller is `trading-agent`'s tool (auto-trade's S-001 routing comment is stale/false) | Given a leg failure after fills, `flattenFilledLegs()` runs and `baskets.status` reflects actual flatten success | `supabase/functions/_shared/basket-logic.test.ts` (16 tests) + `supabase/functions/tests/integration/execute-basket.integration.test.ts` (5 tests, live) | **HIGH-STAKES** — finding #3's flatten-outcome bug is fixed; finding #19 (leg-fill-tracking, also fixed) below; finding #18 (`baskets.user_id` missing from committed migrations) still open |
...
**Coverage gap, stated plainly:** the remaining ~148 feature IDs (every onboarding step, every trading flow, billing, admin observability, all authenticated user journeys) have no functional test yet. Writing them requires a provisioned test account (Supabase auth credentials + a funded paper-mode Kalshi key) that doesn't exist in CI today — creating one is a decision for Onofre (new test credentials, potentially new GitHub secrets), not something to create silently. Existing `tests/e2e/*.spec.ts` cover only the unauthenticated/public surface (landing, login, signup, performance, terms/privacy, basic a11y/security smoke). The 16 findings in §6 were prioritized over broad new test authoring because they were live bugs, not just coverage gaps — see the CLAUDE.md `execution` section on separating "planned" from "tested."
...
| 18 | `baskets` has no `user_id` column in any committed migration — `20260403000000_multi_tenancy.sql`'s multi-tenancy rollout never touched it, and no later migration adds it either — yet `execute-basket` has called `tenantInsertFields(tenant.userId)` (spreading `{user_id: ...}` into its insert) since tenant-scoping was added. A direct query against the live production database (2026-08-06) confirms the column **already exists there** and `execute-basket` inserts succeed in production today — schema drift, production ahead of `origin/dev`'s migration ledger, not a live outage. **Open** — do not add a migration that creates this column; it would conflict with the one already live. Needs someone to reconcile the migration ledger against `information_schema.columns` on production directly (out of scope for a git-only session) and commit the migration that documents what's already there | EF10 `execute-basket` | HIGH-STAKES (schema drift, open) |
| 19 | `execute-basket` counted any `legResult.success === true` as "this leg filled," including a `success:true` response with an unfilled `status:"open"` trade — `execute-trade` returns `success:true` for a resting/unfilled order (no real liquidity at the requested price), not just for a genuine fill. This let a basket report `"completed"` when a leg never actually filled, and could make `flattenFilledLegs` submit a closing order against a position that never existed, opening a new naked position instead of closing one. Confirmed still present in `origin/dev`'s current `execute-basket/index.ts` and `basket-logic.ts` before this fix (verified by reading both files directly, not assumed). **Fixed**: new `didLegFill()` helper requires `trade.status` to be `filled`/`partial`, gating both basket-completion counting and which legs get flattened | EF10 `execute-basket` | HIGH-STAKES (fixed) |
...
- v2 · 2026-08-06 · Ported `execute-basket`'s leg-fill-tracking fix (finding #19) and its test coverage from a stale branch after verifying the bug was still live in `origin/dev`'s current code; documented the `baskets.user_id` schema-drift finding (#18) without attempting a migration (production already has the column; git does not).
