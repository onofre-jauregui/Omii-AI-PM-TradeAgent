...
serve(async (req) => {
  ...
  try {
    ...
    // for evaluateRisk below — a real balance read, not the peak-portfolio
    // proxy, and not the same call as the later insufficient_balance pre-flight
    // (that one only needs cash, this needs cash + open-position value).
    ...
    // Update risk state for this tenant. Reuse the equity figure already fetched
    // above for the concentration check — previously this call passed neither
    // riskSettings nor a real equity reading, so peak_portfolio_value was always
    // seeded from the $500 fallback on the live path regardless of the account's
    // actual configured capital or real balance.
    await updateRiskState(supabase, userId, 0, settings, currentEquityUsd);
