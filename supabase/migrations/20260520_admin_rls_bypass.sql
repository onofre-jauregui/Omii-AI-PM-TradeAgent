...
DROP POLICY IF EXISTS "admin_read_trades" ON public.trades;
CREATE POLICY "admin_read_trades" ON public.trades
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
  );

DROP POLICY IF EXISTS "admin_read_compliance_log" ON public.compliance_log;
...
DROP POLICY IF EXISTS "admin_read_strategies" ON public.strategies;
CREATE POLICY "admin_read_strategies" ON public.strategies
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
  );

DROP POLICY IF EXISTS "admin_read_risk_state" ON public.risk_state;
